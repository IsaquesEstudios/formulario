...
app.post("/ticonnected", (req, res) => {
    ...
    ejs.renderFile(__dirname + "/template/email-number.ejs", { domain: domain, number: number, name: name, email: email, phone: phone, state: state, city: city, branches: branches, companyName: companyName, message: message }, function (err, data) {
        if (err) {
            console.log(err);
        } else {
            let transporter = nodemailer.createTransport({
